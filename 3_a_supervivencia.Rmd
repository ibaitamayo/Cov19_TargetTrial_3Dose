...
```{r}
# cargar lista----------------------------------------------------------------
lista <- readRDS(file.path("Resultados", "lista.RDS"))
...
## se trata de sacar la distribucion de test diaria en los grupos expuesto y no expuesto y representarlo como en la pag7 del apendice [lo haremos para aquellos test en no casos?]

# No casos
dint <- left_join(test220613_cleaned, dplyr::select(base_analisis, id, exposicion, caso)) %>%
  filter(ftes >= ymd(tiempo_0) & caso == 0) %>%
  mutate(day = as.numeric(ftes - ymd(tiempo_0)), 
         sem = cut(day, seq(from = 0, to = 70, by = 7)),
         exposition = factor(exposicion, labels = c("no booster","booster"))) %>%
  filter(day < 71) %>% 
  group_by(sem, exposition) %>%
  summarise(n = n()) %>%
  na.omit() 

pp <- ggplot(dint, 
             aes(x = sem, y = n, fill = exposition)) + geom_col(position = "dodge")   

pp + ggtitle("Excluyendo test diagnostico de casos")    


dint <- left_join(test220613_cleaned, dplyr::select(base_analisis,id,exposicion,caso)) %>%
  filter(ftes >= ymd(tiempo_0)) %>%  
  mutate(day = as.numeric(ftes - ymd(tiempo_0)), 
         sem = cut(day, seq(from = 0, to = 70, by = 7)),
         exposition = factor(exposicion, labels = c("no booster", "booster"))) %>%
  filter(day < 71) %>%
...
pp + ggtitle("test diagnosticos segun exposición a booster")    

##Esta aplicando aqui la fecha 0???
pobana_n_tabla0 <-  test220613_cleaned %>% 
    filter(id %in% base_analisis$id & tpos == FALSE) %>%
    group_by(id) %>% summarise(n_0 = max(n)) %>%
    right_join(base_analisis, by = "id") %>%
    mutate(n_0 = ifelse(is.na(n_0), 0, ifelse(n_0 > 3, 3, n_0)),
           tv2 = as.numeric(ymd(tiempo_0) - fvc2))
