...
f_analisis_lancet <- function(
  ...
  gg1 <- ggsurvplot(fit, data = base_analisis,
           fun = tipo,
    ...
    gg1$plot <- gg1$plot + geom_vline(xintercept = 7, 
                          linetype = "dashed", 
                          size = 0.7,
                          colour = "dimgrey") +
...
gg1 <- f_analisis_lancet(matching_fcv1,
                         tiempo_efecto = 7,
                         title = "A) Infections. Matching for characteristics*")
gg1$eff
...
gg2 <- f_analisis_lancet(matching_fcv1_domi_2,
                         tiempo_efecto = 7,
...
f_analisis_lancet_hosp <- function(
  ...
  gg1 <- ggsurvplot(fit, data = base_analisis,
    ...
    gg1$table <- gg1$table +
      labs(caption = "* Characteristics: age, sex, previous negative test, primary vaccine type and date, health basic zone, migrant status, flu vaccinated in 2020 and in 2021, and the number of people at home.") +
      theme(axis.line = element_blank(),
            axis.title.x = element_blank(),
            axis.ticks = element_blank(),
            axis.text.x = element_blank(),
...
# componer figura--------------------------------------------------------------
gg_up <- cowplot::plot_grid(gg1$gg$plot, gg2$gg$plot,
                   gg1$gg$table, gg2$gg$table, 
                   rel_heights = c(3.5,1), nrow = 2)
    
gg_down <- cowplot::plot_grid(gg3$gg$plot, gg3$gg$table, 
                   rel_heights = c(3.5,1), nrow = 2) +
  theme(plot.margin = margin(0, 6, 0, 6, "cm"))

gg_fig1 <- cowplot::plot_grid(gg_up, gg_down, nrow = 2)

ggsave("Fig1.svg", height = 9, width = 16, units = "in")
