...
f_test0 = function(data,  tiempo_i,  tiempo_f,  tbnp=7) {
  ...
  data = data %>% filter(tiempo_i<=ftes & ftes<=tiempo_f) %>% dplyr::select(id,n,ftes,tpos)
...
long_test <- wide_test %>% gather(res,ftes,test_0:test_1) %>% na.omit() %>% group_by(id) %>% arrange(n)  %>% mutate(no=lag(res)=="test_1") %>% filter(is.na(no) | !no)  %>% dplyr::select(-no) 

## tbt tiempo entre test, eliminar negativos seguidos de positivo en un lapso corto ( parametro: tbnp)
long_test <- long_test %>% mutate(res=as.numeric(str_sub(res,-1)),tbt=ifelse(lead(res)=="1",lead(ftes)-ftes,NA)) %>% filter(is.na(tbt) | tbt<(tbnp +1)) %>%  dplyr::select(-tbt) %>% data.frame()
}
