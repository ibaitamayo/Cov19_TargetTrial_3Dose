...
tt_tibble <- matching_fcv1$tt_tibble
  ...
  tabla_basal_n <- tt_tibble %>%
    mutate(cuentas = "n") %>% tabyl(cuentas, exposicion) %>%
    adorn_totals("col") %>%  
    adorn_percentages("row") %>%
    adorn_pct_formatting(digits = 1) %>%
    adorn_ns("front") %>% 
    rename(Variable = cuentas)
  ...
  tabla_basal <- bind_rows(tabla_basal_n,
                           tabla_basal_edad,
                           tabla_basal_sexo,
                           tabla_basal_n_test,
  ...
  kable(tabla_basal, align = c("c"), escape = FALSE,
      ...
      caption = "Baseline characteristics of tt-person") %>%
  ...
  pack_rows("Total, n (row-wise %)", 1, 1) %>%
  pack_rows("Age group (years), n (%)", 2, 12) %>%
  pack_rows("Sex, n (%)", 13, 14) %>%
  pack_rows("Number of previous SARS-CoV-2 tests, n (%)", 15, 18) %>%
  pack_rows("Primary vaccine schedule, n (%)", 19, 22) %>%
  pack_rows("Booster dose, n (%)", 23, 24) %>%
  pack_rows("Time since primary vaccine schedule, n (%)", 25, 27) %>%  
...
tabla_1_matching_fcv1 <- hacer_tabla_1(
  tt_tibble = matching_fcv1$tt_tibble %>% 
    mutate(exposicion = factor(exposicion, levels = c(0, 1),
                        labels = c("No booster", "booster"))))
