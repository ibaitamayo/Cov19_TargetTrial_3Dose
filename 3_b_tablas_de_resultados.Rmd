...
tabla_basal_sexo <- hacer_basal_discreta(base = matching_fcv1$tt_tibble,
                                         variable = gen,
                                         relabel = c(
                                           "Men",
                                           "Women")
...
tabla_basal_n_test <- hacer_basal_discreta(base = matching_fcv1$tt_tibble,
                                         variable = n_0,
                                         relabel = c(
                                           "0",
                                           "1",
...
tabla_booster_type <- hacer_basal_discreta(base = matching_fcv1$tt_tibble,
                                         variable = tvc3,
                                         relabel = NULL
                                         )
...
matching_fcv1$tt_tibble %>% 
  mutate(t_desde_pauta_completa = date_start - fpauta_90) %>% 
  skim(t_desde_pauta_completa)

matching_fcv1$tt_tibble %>% 
  mutate(t_desde_pauta_completa = case_when(
    date_start - fpauta_90 <= 60)

tabla_t_pauta_comp <- hacer_basal_discreta()
